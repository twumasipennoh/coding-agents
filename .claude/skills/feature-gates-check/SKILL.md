...
> ⚠️ **Call `pipeline-step.sh end feature-gates-check --status ok|fail` before writing any text.** End-before-deliverable rule — the reply must be the final turn with no tool calls after it.
