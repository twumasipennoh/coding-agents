...
2. Run **ALL** test layers defined in `test-commands.md` (unit, integration, e2e, acceptance — every layer listed under `## Layers`). Do NOT skip any layer. Collect the names of any failing tests per layer. **Run each layer as a blocking `Bash` call, in the same turn** — wrap it in `~/.claude/scripts/longrun-tick.sh -i 60 --no-ping -l /tmp/test-baseline-<layer>.log -- <layer-cmd>`. The blocking Bash call is what holds the turn; do NOT stream it via the `Monitor` tool to "wait" for it — `Monitor` returns immediately and would detach the turn. NEVER launch the baseline as a detached background job and end the turn with a "waiting for results" message: under openclaw `/claude` (= `claude -p`, no agent loop after the final turn) nothing re-invokes you when it finishes, so the pipeline stalls forever. See `~/.claude/CLAUDE.md` § "Long-running commands". **Don't assume a real-API-gated test failure inside `npm test` is shell-state leakage** — some projects deliberately pin live-API opt-in vars (e.g. `RUN_REAL_CLAUDE`) to `"1"` by default in a global test-setup file specifically so tests aren't silently skipped on dev machines (a "no-skipped-tests policy"); `env -u`-stripping the var in that case is a no-op and hides the real signal. Check the project's test-setup file for a default-pinning pattern before concluding "leaked env var."
